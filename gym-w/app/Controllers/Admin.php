...
class Admin extends BaseController
{
    ...
    public function newEjercicio()
    {
        ...
        return view('admin/ejercicios/create', $data);
    }
    ...
    public function createEjercicio()
    {
        if (!$this->validate([
            'titulo'      => [
                'rules'  => 'required|min_length[3]|is_unique[ejercicios.titulo]',
                'errors' => ['is_unique' => 'Ya existe un ejercicio con ese nombre.']
            ],
    ...
    public function updateEjercicio($id)
    {
        ...
        if (!$this->validate([
            'titulo'   => [
                'rules'  => "required|is_unique[ejercicios.titulo,id,{$id}]",
                'errors' => ['is_unique' => 'Ya existe otro ejercicio con ese nombre.']
            ],
            'grupo_id' => 'required' 
        ])) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
    ...
    public function createGrupo()
    {
        if (!$this->validate([
            'nombre'      => [
                'rules'  => 'required|min_length[3]|is_unique[grupos_musculares.nombre]',
                'errors' => ['is_unique' => 'Ya existe un grupo muscular con este nombre.']
            ],
            'descripcion' => 'required'
        ])) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
    ...
    public function updateGrupo($id)
    {
        ...
        if (!$this->validate([
            'nombre' => [
                'rules'  => "required|is_unique[grupos_musculares.nombre,id,{$id}]",
                'errors' => ['is_unique' => 'Ya existe otro grupo con este nombre.']
            ]
        ])) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
   ...
   public function noticias() {
    ...
    public function createNoticia()
    {
        if (!$this->validate([
            'titulo'            => [
                'rules'  => 'required|min_length[3]|is_unique[noticias.titulo]',
                'errors' => ['is_unique' => 'Ya existe una noticia con este título.']
            ],
    ...
    public function updateNoticia($id)
    {
        ...
        if (!$this->validate([
            'titulo' => [
                'rules'  => "required|is_unique[noticias.titulo,id,{$id}]",
                'errors' => ['is_unique' => 'Ya existe otra noticia con este título.']
            ],
            'fecha_publicacion' => 'required|valid_date'
        ])) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
