...
$ver_categoria = $conexion -> query("SELECT nombre, codigo FROM sagas;") or die ($conexion->error) ;
...
if (isset($_GET["id_saga"])){
    //recoger variable de URL
    $id_saga = $_GET["id_saga"];


    //ejecutamos Select de libros de la categoria $id_categoria
    $mostrar_sagas = $conexion->query( "SELECT * FROM videojuegos JOIN consolas on videojuegos.consola = consolas.codigo JOIN sagas on videojuegos.saga = sagas.codigo JOIN generos ON videojuegos.genero = generos.codigo WHERE videojuegos.saga = $id_saga;") or die ($conexion->error) ;
} 

else {
    //ejecutamos SELECT de libros
    $mostrar_sagas = $conexion->query( "SELECT * FROM videojuegos JOIN consolas on videojuegos.consola = consolas.codigo JOIN sagas on videojuegos.saga = sagas.codigo JOIN generos ON videojuegos.genero = generos.codigo") or die ($conexion->error) ;
}
?>
...
<body>
    ...
    <header>
        <div class="container">
            ...
            ?>
                    ...
                    <li><a href="sagas.php?id_saga=<?php echo $id_saga;?>"><?php echo $saga;?></a></li>
            <?php } //end while?>
            </ul>
            </nav>
        </div>
    ...
    <div class="main-container container">
        ...
        <main class="main-content">
            ...
            <?php
            while($fila = $mostrar_sagas->fetch_array()){
                    $imagen = $fila[2];
                    $videojuego = $fila[1];
                    $saga = $fila[12];
                    $genero = $fila[16];
