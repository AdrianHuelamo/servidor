...
$ver_categoria = $conexion -> query("SELECT nombre, codigo FROM consolas;") or die ($conexion->error) ;
...
if (isset($_GET["id_consola"])){
    ...
    $mostrar_consolas= $conexion->query( "SELECT * FROM videojuegos JOIN consolas on videojuegos.consola = consolas.codigo JOIN sagas on videojuegos.saga = sagas.codigo JOIN generos ON videojuegos.genero = generos.codigo WHERE videojuegos.consola = $id_consola;") or die ($conexion->error) ;
} 

else {
    //ejecutamos SELECT de libros
    $mostrar_consolas= $conexion->query( "SELECT * FROM videojuegos JOIN consolas on videojuegos.consola = consolas.codigo JOIN sagas on videojuegos.saga = sagas.codigo JOIN generos ON videojuegos.genero = generos.codigo") or die ($conexion->error) ;
}
?>
...
<body>
    ...
    <header>
        <div class="container">
            ...
            ?>
                    ...
                    <li><a href="index.php?id_consola=<?php echo $id_consola;?>"><?php echo $consola;?></a></li>
            <?php } //end while?>
            </ul>
            </nav>
        </div>
    ...
    <div class="main-container container">
        ...
        <main class="main-content">
            ...
            <?php
            while($fila = $mostrar_consolas->fetch_array()){
                    $imagen = $fila[2];
                    $videojuego = $fila[1];
                    $saga = $fila[12];
                    $genero = $fila[16];
